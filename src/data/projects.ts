 ...
 * 4. 实战项目条目不写本地端口号、demo 输出分值等「练手痕迹」，统一以
 *    「服务能力 / 测试结果」口径呈现，避免被面试官识别为玩具项目。
 *
 * 模板约束：
...
export const projects: Resume['projects'] = [
  {
    ...
    role: '项目负责人（独立承担方案设计与后端核心开发）',
    duration: '2024-10 ~ 2025-03',
    description: '公司在承接政务服务数字化项目中，热线咨询与自助查询的 FAQ 知识分散在各业务线、口径不一且上线无灰度手段；据此主导建设统一知识中台，面向政务热线坐席、IM 客服助手与移动 App / Web 自助查询三类入口，由一套后端同时支撑知识生产、融合检索与多渠道问答；通过测试/正式双环境隔离发布让知识上线前可验证、可回滚',
    responsibilities: [
      '【融合检索】依托 Elasticsearch 8.x 单索引单分片架构，通过 Script Score / RRF 算法融合 BM25 全文检索与 dense_vector 语义向量检索，结合 Sentence-Transformers 本地化高性能推理，实现毫秒级高精度双路召回（P99 < 200ms，Top-5 命中率 92%）；调优层面以单分片规避多分片分数归并开销，RRF 按排名融合规避 BM25 与向量分数不可比问题，本地推理规避远程 API 网络往返',
      ...
      '【多渠道答案适配引擎】抽象多视角答案适配引擎，支持按微信、App、Web 等渠道动态下发文本、富文本与交互卡片，同一问题在不同渠道返回差异化答案；预留标准化钉钉/企微 Webhook 接入层，问答能力经统一消息协议无缝嵌入多 IM 业务生态，新渠道接入不改动问答核心链路',
      '【高并发工程吞吐】基于 FastAPI 异步并发框架构建全异步服务链路，SSE 流式响应驱动打字机式交互体验；按 routers / schemas / models / services 分层组织代码，优化 SQLAlchemy 2.0 批量处理链路，支撑 5 万级条目高效导出与 3000 条大文件异步批量导入',
    ],
  },
  {
    ...
    description: '政务政策文件彼此引用关联、群众咨询常需跨文档多跳推理，通用 RAG 在此场景幻觉率高、回答无法溯源到政策原文即不可用；据此打造面向复杂政务场景的 GraphRAG 智能底座，显著压降大模型在政务政策多跳推理中的幻觉率，兼顾高召回率、低计算成本与强可解释性溯源，覆盖「文档解析、重叠分块、向量化、召回、精排、生成」全链路',
    responsibilities: [
      '【三存储异构架构】设计「MySQL（业务元数据）+ Elasticsearch（全文与向量双路）+ Neo4j（政务实体关系图谱）」的异构三存储架构，针对政务场景错综复杂的政策关联与多跳问答需求，通过图文协同检索打通跨文档隐式关联——图谱路径是政策条文联引、上下游修订关系等多跳推理的唯一可靠入口，补齐纯向量检索在语义相似度之外的推理盲区，具备不可替代性',
      '【两阶段检索 + Rerank 工程权衡】混合检索召回 + bge-reranker-base 精排：以 BERT 分类模型对 (query, chunk) 对打分，召回阶段求快、精排阶段求准；并按业务场景决定 Rerank 引入时机——政策答复等高精度场景强制精排提升 Top-5 命中率，通用对话场景仅做混合检索以控制延迟与算力成本',
      ...
      '【模型服务化与私有化合规】embedding / rerank 模型在服务启动时加载一次并以模块级全局实例共享，避免每请求重复加载数 GB 权重带来的内存与耗时开销，支持 cuda / mps / cpu 自适应部署；生成侧大模型经 XInference（vLLM 推理后端）服务化接入，全链路私有化部署满足政务「数据不出域」的合规与安全要求',
    ],
  },
  {
    ...
    description: '政务政策研究与需求调研依赖人工多轮检索整理，单主题耗时 2-3 小时且过程不可回溯；输入研究主题即自动完成多轮迭代调研，产出带来源引用、置信度标注与过程记录的结构化报告，把单主题调研压缩到 8-15 分钟',
    responsibilities: [
      '【Agent 编排】基于 LangGraph 有向图状态机编排 Keyword / Summary / Judge / Report 四角色分工，复用大厂主流工程框架的内置 Checkpointer 机制实现多轮长任务状态持久化与断点续跑，确保控制流确定性、可测试、可复现，语言任务完全下沉至 Agent 节点，避免自研编排框架的维护负担',
      '【迭代收敛】每轮由 JudgeAgent 判断信息是否充分，不足则基于新关键词进入下一轮检索，以 max_rounds 上限保证收敛；单次检索失败兜底为空结果而不中断整体流程',
      '【结构化与会话隔离】依托 Pydantic 与结构化输出自约束模式（schema 注入 function calling 约束 + 解析后二次校验、失败自动修复重试），将大模型格式解析失败率压降至 0.1% 以下；基于 FastAPI + Redis 实现多用户 session 级上下文隔离与会话 TTL 动态内存回收，确保高并发下多租户上下文零污染',
      '【过程可审计】通过 LangGraph 状态快照与 on_progress 回调逐轮记录中间状态（步骤 / 草稿 / 来源），研究过程可落盘、可回放、可断点恢复；来源按 URL 去重，置信度基于来源时间分布确定性计算，无来源结论显式标注为模型推断',
      '【上下文工程】采用 Jinja2 模块化模板集中管理 5 类 Agent 提示词，实现提示词资产版本化（git diff 可读）与控制流完全解耦，支持热更新与按角色独立灰度调优（A/B 不阻塞主流程），大幅降低复杂多智能体系统的提示词维护成本',
    ],
  },
  {
    name: 'mcp-servers',
    ...
    description: 'LLM 应用需要标准化的工具接入协议，而文件 / 数据库 / 企业 API 暴露给模型必须安全可控；据此实现 3 个 MCP（Model Context Protocol）服务器，采用「HTTP JSON-RPC 传输层 → MCP 协议核心 → 业务 Server」三层架构，覆盖协议握手、三原语注册与四道安全防线',
    responsibilities: [
      ...
      '【四道防线与服务化】路径白名单、只读模式、SQL 守卫、Token 鉴权四道防线经 9 组安全回归用例（目录穿越 / 越权写 / 危险 SQL / 无凭证访问）全部拦截验证；经 FastAPI 暴露统一 MCP 端点，token / 白名单 / 只读开关均由环境变量注入，Docker 一键部署',
    ],
  },
  {
    name: 'debate-agents',
    ...
    description: '单 Agent 决策视角单一、易自我确认；据此用「结构化辩论」做决策——正反双方立论-质询-反驳-总结，法官按四维度打分裁决，双层防共识坍缩，让对立观点在规则下对抗并由独立裁判裁决',
    responsibilities: [
      ...
      '【实测闭环与服务化】暴露 /debate HTTP 入口与 /ws/debate 实时通道，实测单场辩论 11 阶段事件全量产出，裁决三态（明确胜出 / 势均力敌转人工 / 坍缩中止）均可复现回放，决策过程可审计',
    ],
  },
  {
    ...
    description: '多租户 SaaS 知识库的核心风险是「绝不能让 A 公司检索到 B 公司的文档」；据此构建「租户上下文 → 分区召回 → ACL 过滤 → 防泄漏后置校验 → LLM 问答 → 审计计费」的纵深防御架构，兼顾数据隔离、商业化合规与可解释引用',
    responsibilities: [
      ...
      '【LLM 网关接口化】LLM 网关经接口抽象提供 Spring AI 在线生成与 Mock 离线回归双实现，「检索 → 生成 → 审计落账」链路不依赖模型服务即可离线编译验证，隔离防线可独立回归',
      '【REST 服务化】提供 /ask /audit /billing 接口，租户与用户上下文统一由 API 网关鉴权后经 X-Tenant-Id / X-User 头注入',
    ],
  },
  {
    name: 'devops-agent',
    ...
    primaryLanguage: ['Python', 'FastAPI', 'ReAct', 'HITL'],
    role: '独立设计与开发',
    duration: '2026-06 ~ 2026-07',
    description: '线上故障排查依赖资深 SRE 人肉串联「指标-日志-变更」三类证据，响应慢且经验难沉淀；据此实现告警触发自动诊断的运维 Agent——查指标、检索日志、分析 K8s 事件、定位根因，ReAct 观察-假设-验证循环 + HITL 安全处置，把 SRE 经验沉淀为可回归的诊断能力',
    responsibilities: [
      ...
      '【证据源适配层解耦】日志 / 指标 / K8s 事件 / 变更四类证据源统一采集接口定义，适配层可插拔对接 Prometheus / ELK / K8s API / CMDB 采集器；Mock 采集器注入「发布后连接池泄漏」确定性故障场景，诊断层零改动即可全链路离线回归',
    ],
  },
  {
    ...
    description: 'K12 辅导中题海战术无差别推送、直接给答案剥夺思考是核心矛盾；据此构建「学生答题 → 判题 → 画像更新 → 薄弱点排序 → 动态难度选题 → 答错引导/归因」的个性化辅导闭环，用掌握度画像驱动因材施教',
    responsibilities: [
      ...
      '【画像可解释与服务化】掌握度、答题数、正确数三本账可查，薄弱知识点自动排序驱动选题；REST 服务支持按画像选题、提交答题、查询画像，全链路离线可运行',
    ],
  },
  {
